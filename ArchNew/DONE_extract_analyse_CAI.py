...
dat = pd.read_csv(os.path.join(root_path,'complete_arch_CDS.dat'))
...
######################################################################
##     REPLACE GenomicID with the assembly_accession !!!!!!!!!!!!   ##
######################################################################
...
dat_with_cai_trop[['assembly_accession','cDNA','fid','pid','product','protein','status','table','ribosomal','CAI','TrOp']].to_csv(os.path.join(path,"complete_arch_CDS_CAI_DNA.dat"),index=False)
...
ax.legend(loc='best')
plt.savefig(os.path.join(path,"org_cai_to_arch.pdf"))
#
# 
plt.clf()
...
ax.set_xlabel("median CAI")
ax.set_ylabel("CAI coefficient of variation") # using plain sigma works worse ...
plt.savefig(os.path.join(path,"org_cai_ribonum_arch.pdf"))
