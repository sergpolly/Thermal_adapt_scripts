...
ribo_cai = pd.read_csv(os.path.join(path,"complete_CDS_CAI_DNA.dat"),low_memory=False)
...
cai = ribo_cai.groupby('GenomicID')

genom_id = cai.groups.keys()
...
dat_with_cai_trop[['GenomicID','cDNA','fid','pid','product','protein','status','table','CAI','TrOp']].to_csv(os.path.join(path,"BOOT_LOWCAI_complete_CDS_CAI_DNA.dat"),index=False)
