...
org_cai_descr = {"GenomicID":idx_for_prot,"TrOp":percentile,"median_cai":median,"mean_cai":mean,"sigma_cai":sigma}
...
dat_with_cai_trop[['GenomicID','cDNA','fid','pid','product','protein','status','table','CAI','TrOp']].to_csv(os.path.join(path,"BOOT_RANDOM_complete_CDS_CAI_DNA.dat"),index=False)
...
# # # 
# plt.clf()
# size_dot = lambda x: 10 if 500<x<3000 else 120
# plt.scatter(x=org_cai_df.median_cai,y=np.true_divide(org_cai_df.sigma_cai,org_cai_df.mean_cai),s=org_cai_df.prot_count.apply(size_dot),c="blue",edgecolor=None)
# ax = plt.gca()
...
# plt.clf()
# plt.plot(org_cai_df.median_cai,org_cai_df.mean_cai,'ro')
# plt.show()
