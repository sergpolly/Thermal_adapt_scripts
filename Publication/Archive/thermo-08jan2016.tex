...
%Correlation profiles yielding overall highest $R_M$ ($w=0.06$) and $R_T$ ($w=0.07$) are depicted with blue and red correspondingly, along with the dashed guiding--lines for the optimal temperatures correspondingly $T^*_M=0.9$ p.u.  and $T^*_T=1.2$ p.u. Simulations are also compared with the average amino acid composition (b), $R_A$, and with the slopes of amino acid temperature trends (d), $R_В}$. Correlation profiles corresponding to the overall optimal trade--off parameter $w^*=0.07$ are plotted in blue, and the unified optimal temperature range $1.0=T^**_M<T<T^**_T=1.2$ is highlighted in yellow. Optimal temperature range cover highest $R_A$ and $R_В}$ values.
}
